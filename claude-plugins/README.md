...
```bash
curl -fsSL https://daemux.ai/claude-plugin/install | sh
```

**Global (all projects):** Install once for all projects:

```bash
curl -fsSL https://daemux.ai/claude-plugin/install | sh -s -- --global
```
...
```bash
curl -fsSL https://daemux.ai/claude-plugin/install | sh -s -- --uninstall
```

**Global:** Remove from all projects and clean up marketplace:

```bash
curl -fsSL https://daemux.ai/claude-plugin/install | sh -s -- --global --uninstall
```
