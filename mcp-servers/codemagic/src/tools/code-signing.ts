...
export function registerCodeSigningTools(server: McpServer, v3: V3Client): void {
  server.tool(
    ...
    ({ teamId, groupName, keyIdentifier, issuerId, privateKey }) =>
      handleToolCall(() =>
        createSecureVariableGroup(v3, teamId, groupName ?? 'ASC Credentials', [
  ...
  server.tool(
    ...
    ({ teamId, groupName, certificate, certificatePassword, provisioningProfile }) =>
      handleToolCall(() =>
        createSecureVariableGroup(v3, teamId, groupName ?? 'Code Signing', [
